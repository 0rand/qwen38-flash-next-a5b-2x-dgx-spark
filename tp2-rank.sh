...
NCCL_IB_HCA="${NCCL_IB_HCA:?}"; FP8_HYBRID="${FP8_HYBRID:-1}"; EXTRA_ARGS="${EXTRA_ARGS:-}"; NCCL_SOCKET_IFNAME="${NCCL_SOCKET_IFNAME:?}"; CONTROL_IF="${CONTROL_IF:?}"
...
docker run -d --name "$CONTAINER" --gpus all --ipc=host --shm-size "$SHM" --network host \
  ...
  --kv-cache-memory-bytes "$KV_BYTES" --kv-cache-dtype "$KV_DTYPE" \
